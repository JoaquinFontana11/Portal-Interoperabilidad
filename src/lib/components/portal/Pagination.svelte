<script lang="ts">
	...
	const allPage: number[] = [];
	for (let i = value; i < maxPage; i++) allPage.push(i);
</script>
...
<nav aria-label="Page navigation example">
	<ul class="inline-flex items-center -space-x-px">
		...
		{#each allPage as pageNumber}
			{#if (value + 1 >= pageNumber && value - 1 <= pageNumber) || (value == 0 && value + 2 >= pageNumber) || (value == maxPage - 1 && value - 2 <= pageNumber)}
				<li>
					<button
